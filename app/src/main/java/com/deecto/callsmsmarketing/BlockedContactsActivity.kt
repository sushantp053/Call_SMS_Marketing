        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == CONTACT_PICK_CODE) {
                ...
                if (cursor1.moveToFirst()) {
                    val contractsWise = cursor1?.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME)
                }
                if (cursor1.moveToFirst()){
                    //get contact details
                    val contactId = cursor1.getString(cursor1.getColumnIndexOrThrow(ContactsContract.Contacts._ID))
//                    val contactId = cursor1.getColumnIndex(ContactsContract.Contacts._ID)
                    val contactName = cursor1.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME)
                    val contactThumbnail = cursor1.getColumnIndex(ContactsContract.Contacts.PHOTO_THUMBNAIL_URI)
                    val idResults = cursor1.getColumnIndex(ContactsContract.Contacts.HAS_PHONE_NUMBER)
                    ...
                    if (idResultHold == 1){
                        ...
                        while (cursor2!!.moveToNext()){
                            //get phone number
                            val contactNumber = cursor2.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NUMBER)
                            //set phone number
